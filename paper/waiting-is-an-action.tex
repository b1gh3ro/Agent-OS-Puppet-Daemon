...
one in $82\%$ of waits. In a 19.1-hour, 63-run deployment, $32\%$ of active agent
time was spent waiting, so the pattern is not a corner case.
\end{abstract}
...
waiting is negligible. In deployment it is not: in the 63-run deployment we
describe in Section~\ref{sec:eval}, waiting consumed 32\% of active agent time.
...
monotonic clock, never as differences of wall-clock timestamps. This matters. In
our deployment logs (\S\ref{sec:rq3}), host suspend inflated wall-clock spans by
22 hours across 63 runs (54\% of the naive total), while in-process durations
were unaffected. A wait that reports 19\,s of elapsed loop
...
pixel diff. This is the missed-change side of the threshold (\S\ref{sec:rq2}), and
...
filters, \S\ref{sec:waiting}) in exchange for never missing a signal, the
...
The main observation in Table~\ref{tab:deploy} is that $32\%$ of active agent time
was spent waiting. Waiting is not a corner case in deployment even though it is
absent from benchmarks (\S\ref{sec:gap}).

We do not interpret the 359-to-7 ratio of event-driven to fixed waits as evidence
of a preference. The tool descriptions presented to the model
...
chose event-driven waiting in $82\%$ of waits ($73$ of $89$), and free-choice
...
false wake is an unbounded-rate model call (\S\ref{sec:cost}). The primitive is
...
sandbox with no host filesystem access. The controlled experiments
(\S\ref{sec:rq1},~\S\ref{sec:rq2}) are entirely synthetic: scripted dialogs,
clocks, and terminals with no accounts, no personal data, and no human subjects.
The deployment logs behind \S\ref{sec:rq3}, however, arose from personal use and
contain third-party conversation data from people who did not consent to
appearing in a research artifact. We therefore report only aggregate counts from
those logs, never message content, and withhold the raw logs entirely; the
released artifact comprises the code and the synthetic task suite alone. The motivating example opening Section~\ref{sec:intro} is synthetic for
the same reason.
