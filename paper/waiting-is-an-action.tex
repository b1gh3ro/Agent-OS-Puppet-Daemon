...
characterize rather than hide. \todo{headline result from Section V}
...
\todo{Our own two-call probe with an identical 5{,}531-token prefix billed zero
cached tokens, failing to reproduce a caching effect. Whether implicit caching
materializes under the real workload is an open question that \S\ref{sec:rq1}
must settle. Do not assert a caching effect until measured.}
...
\todo{This section is the critical path. \S\ref{sec:rq1} and \S\ref{sec:rq2}
report no results yet; the protocol below is fixed in advance and the
experiments have not been run. \S\ref{sec:rq3} contains real data.}

\subsection{Setup}
\label{sec:setup}
\textbf{Task suite.} Six synthetic waiting tasks with known ground truth: (i) a
terminal command (\texttt{sleep 90 \&\& echo done}); (ii) a large file download
with a progress indicator; (iii) a spinner that resolves after a fixed delay;
(iv) a dialog that appears after a delay; (v) a visible countdown timer (a
periodic-noise case); and (vi) a no-change control that never resolves, to
measure timeout behavior. Ground-truth completion time is known for each,
allowing reaction latency to be computed exactly.

\textbf{Conditions.} Three strategies (poll, \texttt{sleep}, event-driven)
$\times$ six tasks $\times$ five repeats.
...
\label{sec:rq1}
\todo{Fig.~2, a cost--latency plot. Prediction, registered here in advance:
polling achieves low latency at $O(T/p)$ calls; \texttt{sleep} achieves $O(1)$
calls but latency proportional to overshoot; event-driven achieves both on
quiescent tasks (i--iv) and degrades on the countdown task (v). If the prediction
fails --- in particular if event-driven latency is worse than the 1\,Hz sampling
bound suggests --- report it.}

\subsection{RQ2: change-threshold characterization}
\label{sec:rq2}
\todo{Fig.~3, false wakes versus missed changes as $\phi$ (\texttt{\_CHANGE\_%
FRACTION}) sweeps, against a scene with known noise (countdown ticking) and known
signal (dialog appears). This locates the boundary between the $O(1)$ and $O(rT)$
regimes of \S\ref{sec:cost} and either justifies $\phi = 0.004$ or shows it is
wrong --- the latter is equally publishable.}
...
therefore measures instruction-following, not a discovered property.
\todo{To make any claim about which primitive an agent selects, run an arm with
a neutral prompt that describes both tools without recommending either.}
