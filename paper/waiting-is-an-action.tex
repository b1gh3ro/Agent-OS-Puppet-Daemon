...
median of $3$ model calls against polling's $7$, then rises to $5$--$6$ calls
...
one in $84\%$ of waits. In a 19.1-hour, 63-run deployment, $32\%$ of active agent
...
\texttt{sleep} reacts in $45.7$\,s, the overshoot of a fixed guess. Event-driven
...
degrades, to $5$ calls at $1.54\times$ and $6$ at $7.87\times$, climbing toward
polling's $7$ as $O(1)\!\to\!O(rT)$ predicts. The transition sits where the ladder
crosses $\phi$, so the threshold governs the trade-off rather than merely bounding
it.
...
round-trip that follows adds a median of $5.4$\,s, for $7.5$\,s end-to-end. That
...
Polling and \texttt{sleep} never fail; event-driven and free-choice succeed $97\%$
and $94\%$ of the time. The rare miss is the eager detector's failure mode: a wake
