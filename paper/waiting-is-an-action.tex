...
\author{
\IEEEauthorblockN{Mohammed Saad}
\IEEEauthorblockA{\textit{Dept. of CSE} \\
\textit{UCE, Osmania University}\\
Hyderabad, India \\
saadmujeeb1008@gmail.com}
\and
\IEEEauthorblockN{Mohammed Mujtabaa Hussain}
\IEEEauthorblockA{\textit{Dept. of CSE} \\
\textit{UCE, Osmania University}\\
Hyderabad, India \\
mujtabaawork17@gmail.com}
\and
\IEEEauthorblockN{Mohd Abdul Hameed}
\IEEEauthorblockA{\textit{Asst. Prof., Dept. of CSE} \\
\textit{UCE, Osmania University}\\
Hyderabad, India \\
professor.hameed@gmail.com}
}
...
for a reply, a download, a build, or a person. Current agent architectures have
no way to express duration. The ReAct loop models an agent as a sequence of
actions, so an agent that must wait can only poll: emit a no-op action and
re-examine the screen. When the poller is a language model, each poll costs a
full model round-trip, so the cost of waiting scales with wall-clock duration.
Browser automation solved this long ago with event-driven primitives such as
Playwright's \texttt{wait\_for\_selector}, but those require a DOM, and an agent
that perceives only pixels has no equivalent. We contribute the pixel-level
analogue: waiting as a first-class action, with fixed (\texttt{sleep}) and
event-driven (\texttt{wait\_for\_screen\_change}) primitives evaluated
server-side, outside the model loop. Event-driven waiting reduces waiting from
$O(T/p)$ model calls to $O(1)$ on a quiescent screen, and degrades toward polling
as screen activity rises. Across a controlled experiment of four waiting
strategies over eight screens, it matches polling's sub-10-second reaction at a
median of $3$ model calls against polling's $7$, then rises to $6$--$7$ calls
once on-screen noise crosses the detector's threshold. Given both a fixed and an
event-driven primitive under a neutral prompt, the agent chose the event-driven
one in $82\%$ of waits.
\end{abstract}
...
reply arrives in thirty minutes. The agent's actual work, reading the message,
drafting an answer, and sending it, takes four actions. The other thirty minutes
are spent doing nothing, and that is the expensive part.

This is not the setting that GUI agent research measures. Benchmarks such as
...
neither wall-clock duration nor the token cost of waiting, because for such tasks
waiting is negligible. In real use it is not: an agent can spend a large share of
its active time waiting, and that time is exactly what these benchmarks omit.
...
\begin{itemize}
  ...
  \item \textbf{C3.} A design for agent-initiated human handoff, for waits that
  no detector can resolve (Section~\ref{sec:handoff}).
\end{itemize}
...
expressed over pixels instead. This introduces a false-wake failure mode that the
DOM formulation does not have, which Section~\ref{sec:waiting} makes explicit.
...
complete in minutes, and it is why the long-horizon cost of waiting remains
...
We evaluate within a small agent runtime. The runtime is not a contribution of
this paper; it follows established practice, and we describe it only enough to
...
One design property matters for what follows: screenshots are captured on
...
reasons and emits an action. The \emph{wait} loop (right) is the new part:
...
\algline{2}{$t_0 \leftarrow$ MonotonicNow() \hfill $\triangleright$ not wall-clock; Section~\ref{sec:setup}}
...
activity (a ticking clock, an animated spinner, an unrelated notification) false
wakes arrive at some rate $r$, giving $F \approx rT$ and a cost of $O(rT)$. In the
limit of continuous motion (video playback), event-driven waiting fires every
frame and \emph{collapses into polling, but worse}: every wake is a full model
call, with no interval $p$ to bound the rate. The claim is therefore $O(1)$ model
calls on a quiescent screen, degrading toward polling as screen activity rises.
Section~\ref{sec:rq2} measures where that boundary lies.
...
$\lceil T/p \rceil$, since caching does not remove round-trips; (ii) each call
still pays its uncached delta, which includes the new screenshot, the expensive
part; and (iii) each call still pays its full latency. We therefore instrument and
...
Two controlled experiments back the claims: Section~\ref{sec:rq1}, the cost--latency
comparison across strategies, and Section~\ref{sec:rq2}, the threshold sweep.
...
(Section~\ref{sec:rq2}). One is a \emph{no-change control} that never resolves, to
...
free-choice arm that exposes both \texttt{sleep} and event-driven described
neutrally. Each leaves the model exactly one way to express a wait (the free arm,
two), so that any difference is architectural rather than a prompting effect. The
...
monotonic clock, never as differences of wall-clock timestamps. This matters on a
suspendable host: if the machine sleeps mid-wait, a loop that ran for seconds can
span hours of wall-clock, while the in-process duration is unaffected. \emph{On a
...
\midrule
poll               & 7 & 8.3  & 100\% \\
\texttt{sleep}     & 3 & 45.3 & 100\% \\
event-driven       & 3 & 7.9  & 95\%  \\
free choice        & 3 & 8.6  & 92\%  \\
\bottomrule
\end{tabular}
\end{table}
...
Polling spends a median of $7$ model calls per wait, while \texttt{sleep},
event-driven, and free-choice each spend $3$. Reaction latency separates them the
other way: polling and event-driven both react in under $9$\,s, while
\texttt{sleep} reacts in $45.3$\,s, the overshoot of a fixed guess. Event-driven
waiting lands where the cost model predicted, matching polling's responsiveness at
less than half its model calls (Fig.~\ref{fig:rq1}a).

Fig.~\ref{fig:rq1}b sweeps the event-driven arm across the noise ladder. Below the
detector threshold, at $0.26\times$ and $0.58\times\phi$, it holds the $O(1)$
floor of $3$ calls, indistinguishable from a still screen: motion that
\emph{looks} busy but stays under $\phi$ costs nothing. Above the threshold it
degrades, to $6$ calls at $1.54\times$ and $7$ at $7.87\times$, reaching polling's
$7$ as $O(1)\!\to\!O(rT)$ predicts. The transition sits where the ladder crosses
$\phi$, so the threshold governs the trade-off rather than merely bounding it.

The reaction figure is end-to-end, from the ground-truth change to the first agent
action, so it folds in one Gemini round-trip rather than detection alone.
Decomposing the event-driven runs, the detection component (change to the watcher
waking) has a median of $2.1$\,s, near the $1$\,Hz sampling bound, while the model
round-trip that follows adds a median of $5.6$\,s, for $7.7$\,s end-to-end. That
model term is a fixed cost of consulting a language model and is shared across all
arms, so it inflates the absolute numbers without biasing the comparison. The
architecture controls only the $2.1$\,s detection term.

Polling and \texttt{sleep} never fail; event-driven and free-choice succeed $95\%$
and $92\%$ of the time. Every one of the five misses was a timeout on the two
scenes with the subtlest change, the terminal build (a single completion line) and
the download (a banner swap), where the pixel change occasionally fell below $\phi$
and the detector never woke. Polling and \texttt{sleep} avoid this because they
re-read the whole screen with the model on a schedule rather than depending on the
pixel diff. This is the missed-change side of the threshold (Section~\ref{sec:rq2}), and
the price of the $O(1)$ cost.

The free-choice arm exposes both \texttt{sleep} and event-driven waiting with
neutral descriptions and no recommendation, to see which the model selects on its
own. It chose event-driven waiting in $82\%$ of waits ($73$ of $89$), and matched
the event-driven arm on cost and latency (Table~\ref{tab:rq1}). The preference
does not depend on a prompt that recommends one tool, though under a single model
we claim it no further.
...
offline at every $\phi$, counting false wakes on the noise and testing whether the
real event still trips the detector. Fig.~\ref{fig:rq2} shows the result.
...
Section~\ref{sec:cost} rather than merely bounding it: a scene is quiescent-in-effect
precisely when its amplitude sits below $\phi$. Second, the signal changes
$33.6\times\phi_0$ of the screen, far above every noise scene, so any $\phi$ in
the band $[\,1.4\times,\,33.6\times\,]\phi_0$ eliminates the clock's false wakes
while still catching the event. The production $\phi_0$ sits at the eager end of
the safe range: it admits some false wakes on the busier scenes (which the model
filters, Section~\ref{sec:waiting}) in exchange for never missing a signal, the
asymmetry we intended. Raising $\phi$ toward the signal amplitude would trade
those false wakes for missed events on any scene whose real signal is smaller than
the banner used here, so we do not; a fixed scalar $\phi$ is a starting point,
...
restarting the task. We present this as a design element that completes the set of
waiting primitives, alongside \texttt{sleep} and \texttt{wait\_for\_screen\_change};
we make no autonomy claim for it.
...
false wake is an unbounded-rate model call (Section~\ref{sec:cost}). The primitive is
correct only for screens that are quiescent between meaningful events.
...
sandbox with no host filesystem access. All experiments
(Sections~\ref{sec:rq1} and~\ref{sec:rq2}) are entirely synthetic: scripted dialogs,
clocks, and terminals with no accounts, no personal data, and no human subjects.
The released artifact comprises the code and the synthetic task suite. The
motivating example opening Section~\ref{sec:intro} is synthetic as well.
...
quiescent screen, with a false-wake term that grows as that assumption weakens.
Characterizing that trade-off, more than the primitive itself, is what this paper
offers.
